        ...
        if tree['m_Name'] == "TrainerTable":
            ...
            for trainer in tree[trainerPoke][1:]:
                ...
                for point in range(len(breakpoints) - 1):
                    ...
                    # print(secondLine)
                    try:
                        trainer["P"f"{pokeNum}MonsNo"] = name(firstLine.split(" ")[0])
                    except:
                        ##Pokemon Like Mr. Mime exist with a space in between AND IT RUINS EVERYTHING AAAAAAAAAAAAAAA
                        
                        trainer["P"f"{pokeNum}MonsNo"] = name(firstLine.split(" ")[0] + " " + firstLine.split(" ")[1])
                    ##Gender
                    if "(" in firstLine:
                        index = firstLine.index("(")
                        trainer["P"f"{pokeNum}Sex"] = genderList.index(firstLine[index:index + 3])
                    ##Item
                    if "@" in firstLine:
                        index = firstLine.index("@")
                        trainer["P"f"{pokeNum}Item"] = itemList.index(firstLine[index + 1:].strip())
                        
                    ##Ability
                    trainer["P"f"{pokeNum}Tokusei"] = ability(secondLine)
